package com.jason.crawler.util;
 ...
 * proxy ip util
 *
 * @author jason 2017-11-08 13:06:55
 */
public class ProxyIpUtil {
    ...
    public static int checkProxy(Proxy proxy, String validSite){
        try {
            URL url = new URL(validSite!=null?validSite: CrawlerConf.SITE_BAIDU);

            HttpURLConnection httpURLConnection = (HttpURLConnection) url.openConnection(proxy);
            httpURLConnection.setRequestProperty("User-Agent", CrawlerConf.USER_AGENT_CHROME);
            httpURLConnection.setConnectTimeout(CrawlerConf.TIMEOUT_MILLIS_DEFAULT);
            httpURLConnection.setReadTimeout(CrawlerConf.TIMEOUT_MILLIS_DEFAULT);
